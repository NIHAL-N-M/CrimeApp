...
try:
    bgg = PIL.Image.open("Images/bg.png")
...
if pimg:
    ...
    # Set background color if image not available
    window.configure(bg="#051729")
...
try:
    ...
    player = tkvideo("Images/Face-recognition.mp4", video_label,loop = 1, size = (400, 200))
    player.play()
except:
    # If video not found, add a placeholder label
    video_label = Label(window, text="Video not available", fg="white", bg="#051729", font="Verdana 12")
    video_label.pack(padx=10,pady=10)
...
try:
    ...
    logo2 = ImageTk.PhotoImage(file = "Images/github.jpg")
    ...
    logo3 = ImageTk.PhotoImage(file = "Images/twitter.jpg")
    tk.Button(window, image=logo3,bg ="#051729",highlightbackground="#051729", highlightthickness=2, command=callback).place(x=30,y=500)
except:
    # If social media logos not found, add text buttons
    tk.Button(window, text="LinkedIn", bg="#051729", fg="white", command=callback2).place(x=30,y=200)
    tk.Button(window, text="Instagram", bg="#051729", fg="white", command=callback3).place(x=30,y=300)
    tk.Button(window, text="GitHub", bg="#051729", fg="white", command=callback1).place(x=30,y=400)
    tk.Button(window, text="Twitter", bg="#051729", fg="white", command=callback).place(x=30,y=500)
